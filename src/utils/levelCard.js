...
async function createLevelCardBuffer({
  ...
  if (title) drawPixelText(ctx, title, 365, 92, 6, accent, "left");

  const nameSize = fitPixelText(safeName, 430, 10, 5);
  drawPixelText(ctx, safeName, 365, 165, nameSize, "#ffffff", "left");

  if (discriminator && discriminator !== "0") {
    const nameWidth = measurePixelText(safeName, nameSize);
    drawPixelText(ctx, `#${discriminator}`, 365 + nameWidth + 16, 172, 6, "#ffffff", "left", 0.4);
  }

  drawPixelText(ctx, "RANK", 825, 88, 6, "#ffffff", "right", 0.75);
  drawPixelText(ctx, `#${safeRank}`, 985, 88, 12, "#ffffff", "right");
  drawPixelText(ctx, "LEVEL", 1080, 88, 6, "#ffffff", "right", 0.75);
  drawPixelText(ctx, String(safeLevel).padStart(2, "0"), 1160, 88, 12, "#ffffff", "right");

  drawPixelText(ctx, safeCurrentXp.toLocaleString(), 870, 205, 6, "#ffffff", "right");
  drawPixelText(ctx, `/${safeNeededXp.toLocaleString()} XP`, 1060, 205, 6, "#ffffff", "right", 0.45);

  drawProgressBar(ctx, 365, 250, 720, 54, percent, accent);
  drawPixelText(ctx, `TOTAL: ${safeTotalXp.toLocaleString()} XP`, 725, 330, 5, "#ffffff", "center");
