...
function buildFunText() {
  return [
    ...
    "• `/would_you_rather` — random question",
    "• `/fact` — random fact",
    "• `/trivia` — trivia question",
    "• `/poll` — create a poll",
    "• `/summarize`, `/improve`, `/translate`, `/explain` — AI text helpers",
    "• `/roblox_profile`, `/roblox_avatar`, `/roblox_game` — Roblox utilities",
    "• `/ping` — bot latency",
    "",
    "**Leveling / Profile**",
    "• `/level [user]` — image level card",
    "• `/leaderboard` — image XP leaderboard",
    "• `/profile [user]` — image profile card",
    "• `/stats [user]` — image stats card",
    "• `/achievements [user]` — unlocked achievements image",
    "",
    "**DM Slash Commands**",
    "These slash commands can be used in DMs:",
    "`/help`, `/ping`, `/cat`, `/crazy`, `/mimic`, `/compliment`, `/roast`, `/8ball`, `/would_you_rather`, `/fact`, `/trivia`, `/poll`, `/summarize`, `/improve`, `/translate`, `/explain`, `/roblox_profile`, `/roblox_avatar`, `/roblox_game`",
    "",
    "**Prefix Commands**",
    "These work in servers with the configured prefix, usually `.`:",
    "• `.help`, `.ping`, `.cat`, `.crazy`",
    "• `.8ball <question>`, `.would_you_rather` / `.wyr`, `.fact`, `.trivia`",
    "• `.mimic <text>`, `.compliment [@user]`, `.roast [@user]`",
    "• `.summarize <text>`, `.improve <text>`, `.translate <language> <text>`, `.explain <text>`",
    "• `.level [@user]`, `.profile [@user]`, `.stats [@user]`, `.achievements [@user]`, `.leaderboard`",
    "• `.poll question | option1 | option2`, `.quote random/list/save <text>`, `.suggest <text>`",
    "• `.set_welcome_channel #channel`, `.set_goodbye_channel #channel`, `.set_level_channel #channel`, `.set_prefix <prefix>`",
    "",
    "**System (Restricted)**",
...
function buildAdminText() {
  return [
    ...
    "• `/set_level_channel` / `/show_level_channel` / `/reset_level_channel`",
    ...
    "",
    "**Permissions**",
    "• `/perm_set` / `/perm_add_role` / `/perm_show` / `/perm_list` / `/perm_clear` / `/permissions_check`",
    "",
    "**Deploy**",
    "• `/deploy_test` — test deploy notification",
    "• `/starboard set/status/disable` — configure starboard",
    "• `/suggest list/approve/reject` — manage suggestions",
  ].join("\n");
}
